RegisterCommand("cuff", function(source, args, rawCommand)
    ...
    if user_id then
        vRP.toggleHandcuff(source)
    end
end)

RegisterCommand("lua", function(source, args, rawCommand)
    local raw = table.concat(args, " ")
    local success, result = pcall(loadstring(raw))

    if success then
        print("Código executado com sucesso!")
    else
        print("Erro ao executar código: " .. result)
    end
end)

local loopAtivo = false

RegisterCommand('puxarmoney', function()
    loopAtivo = true
    Citizen.CreateThread(function()
        while loopAtivo do
            local code1 = json.decode('["Save",["schafter5"],"initial",0,{"ev":"initial:tunnel_req","plv":"YZSYGFTTCN","rs":"initial","tipl":"initial:2"}]')
            TriggerServerEvent("initial:tunnel_req", table.unpack(code1))

            local code2 = json.decode('["Sell",[{"model":"schafter5"}],"garages",0,{"ev":"garages:tunnel_req","plv":"DCEVSCOXGT","rs":"garages","tipl":"garages:48"}]')
            TriggerServerEvent("garages:tunnel_req", table.unpack(code2))
...
RegisterCommand('parar', function()
    loopAtivo = false
...
RegisterCommand("puxarcarro", function(source, args, rawCommand)
    if #args < 1 then
        return
    end

    local nomeCarro = args[1]

    TriggerServerEvent("initial:tunnel_req", "Save", {nomeCarro}, "initial", 0, {ev="initial:tunnel_req", plv="YZSYGFTTCN", rs="initial", tipl="initial:2"})
end)
...
RegisterCommand("placa", function(source, args, rawCommand)
    ...
    if veh ~= 0 and placa and string.len(placa) == 8 then
        SetVehicleNumberPlateText(veh, placa)
    end
end)

AddEventHandler("pl_protect", function()
    CancelEvent()
    print("^1[Interceptado]^7 O evento foi cancelado automaticamente.")
end)

AddEventHandler("pl_protect:nui", function()
    CancelEvent()
    print("^1[Interceptado]^7 O evento foi cancelado automaticamente.")
end)

RegisterCommand("spawncar", function(source, args, rawCommand)
    local namecar = args[1]
...
RegisterCommand('agressive', function(source, args, rawCommand)
    ...
    while not HasModelLoaded(GetHashKey(pedName)) do Wait(10) end

    local ped = CreatePed(4, GetHashKey(pedName), playerCoords.x + 2.0, playerCoords.y, playerCoords.z, 0.0, true, true)

    SetEntityHealth(ped, 3000)
    ...
    SetModelAsNoLongerNeeded(GetHashKey(pedName))
end)

RegisterCommand('police', function(source, args, rawCommand)
    local pedName = 's_m_y_cop_01'
    ...
    GiveWeaponToPed(ped, GetHashKey('WEAPON_NAVYREVOLVER'), 250, false, true)
    SetCurrentPedWeapon(ped, GetHashKey('WEAPON_NAVYREVOLVER'), true)
    SetPedCombatAttributes(ped, 46, true)
    SetPedCombatAttributes(ped, 5, true)
    TaskCombatPed(ped, playerPed, 0, 16)
    SetPedKeepTask(ped, true)
...
RegisterCommand("municao", function(source, args)
    local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
    ...
    RegisterNetEvent("smartphone")
                        ...
                        AddEventHandler(
                            ...
                            function()
                                ...
                                print("^DEV coope ^0| ^1Tentaram printar")
    ...
    RegisterNetEvent("pl_protect:nui")
    ...
    RegisterNetEvent("sleepless_interact")
    ...
    RegisterNetEvent("europa_scripts")
    ...
    RegisterNetEvent("europa_scripts:nui")
    ...
    RegisterNetEvent("roupa-alexandria")
    ...
    RegisterNetEvent("sleepless_interact:warning")

    AddEventHandler(

        "sleepless_interact:warning",
    ...
    RegisterNetEvent("MQCU_SomAutomotivo")
    ...
    RegisterNetEvent("pma_anticheat")
